...
class Main:
    def __init__(self) -> None:
        ...
        self.window_hierarchy = [
            len(self.windows) - i for i in range(len(self.windows))
        ]
    ...
    def validate_overlaping(self, win_1_loc, win_2):
        ...
        def validate_y():
            return (
                win_1_loc[1] < win_2[1] + self.size_y
                and win_1_loc[1] + self.size_y > win_2[1]
            )
            ...
            if ball_position[0] + win_1.balls[i].velocity[0] >= win_1_point[0]:
                ...
                if self.is_top_right(win_1_loc, win_2_loc):
                    ...
                    if win_1.last_active > win_2.last_active:
                        if (
                            self.size_y - ball_position[1] > win_1_point[1]
                            and (ball_position[1]) + win_1.balls[i].velocity[1]
                            > (self.size_y - win_1_point[1]) - win_1.balls[i].size
                        ):
                            a = 1
                            ...
                            a = 2
                            y = self.size_y - (
                                (self.size_y - ball_position[1])
                                + (self.size_y - win_1_point[1])
                            )
                        ...
                        if (
                            win_1_loc[1] + (self.size_y - ball_position[1])
                            < win_2_loc[1] + self.size_y
                            and (ball_position[1]) + win_1.balls[i].velocity[1]
                            >= self.size_y - win_1.balls[i].size - 1
                        ):
                            a = 3
                            y = self.size_y - win_2_point[1]
                            x = ball_position[0] - win_1_point[0]

                        elif (ball_position[0]) + win_1.balls[i].velocity[
                            0
                        ] >= self.size_x - win_1.balls[i].size and win_1_loc[1] + (
                            self.size_y - ball_position[1]
                        ) < win_2_loc[
                            1
                        ] + self.size_y:
                ...
                if self.is_bottom_right(win_1_loc, win_2_loc):
                    ...
                    if win_1.last_active > win_2.last_active:
                        ...
                        if (
                            self.size_y - ball_position[1] < win_1_point[1]
                            and (self.size_y - ball_position[1])
                            + -(win_1.balls[i].velocity[1])
                            > win_1_point[1] - win_1.balls[i].size
                        ):
                            a = 5
                ...
                if x is not None and y is not None:
                    balls_to_transfer.append(
                        (i, x, y, win_1.balls[i].velocity, win_1.balls[i].color)
                    )
            ...
            if ball_position[0] + win_2.balls[i].velocity[0] <= win_2_point[0]:
                ...
                if self.is_top_right(win_1_loc, win_2_loc):
                    ...
                    if win_1.last_active < win_2.last_active:
                        # enters from top
                        if (
                            self.size_y - ball_position[1] < win_2_point[1]
                            and (self.size_y - ball_position[1])
                            + -win_2.balls[i].velocity[1]
                            > win_2_point[1]
                        ):
                            a = 1
                        ...
                        if (
                            ball_position[1] + win_2.balls[i].velocity[1]
                        ) <= win_2.balls[i].size:
                            a = 3
                            x = ball_position[0] + win_1_point[0]
                        ...
                        # and ball is below the window 1 points in window 2
                            ...
                            ball_position[0] + win_2.balls[i].velocity[0]
                            <= win_2.balls[i].size
                            and win_2_loc[1] + (self.size_y - ball_position[1])
                            >= win_1_loc[1]
                        ):
                            a = 4
                    ...
                    if win_1.last_active < win_2.last_active:
                        if (
                            self.size_y - ball_position[1]
                            > win_2_point[1] - win_2.balls[i].size
                            and self.size_y
                            - (ball_position[1] + win_2.balls[i].velocity[1])
                            <= win_2_point[1]
                        ):
                        ...
                        if (
                            ball_position[1] + win_2.balls[i].velocity[1]
                            >= self.size_y - win_2.balls[i].size
                        ):
                            a = 7
                            x = ball_position[0] + win_1_point[0]
                            y = self.size_y - win_1_point[1]
                ...
                if x is not None and y is not None:
                    balls_to_transfer.append(
                        (i, x, y, win_2.balls[i].velocity, win_2.balls[i].color)
                    )
    ...
    def update(self, dt: float) -> None:
        ...
        for wins in self.windows:
            ...
            if len(points) > 4:
                self.balls_passing_windows(points[4], points[5], points[6], points[7])
            if len(points) > 0:
